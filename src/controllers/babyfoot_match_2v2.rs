...
pub struct BabyfootMatch2v2Controller;

#[async_trait]
impl SlashCommandControllerTrait for BabyfootMatch2v2Controller {
    async fn run(
        command: &ApplicationCommandInteraction,
        ctx: &Context,
        app: &ApplicationContext,
    ) -> Result<()> {
        debug!("Entering babyfoot match 2v2 controller...");
        let options = &command.data.options;

        let j1 = discord::resolve_user_arg(options.get(0).unwrap())?;
        let j2 = discord::resolve_user_arg(options.get(1).unwrap())?;
        let score_j1j2: i64 = discord::resolve_int_arg(options.get(2).unwrap())?;

        let j3 = discord::resolve_user_arg(options.get(3).unwrap())?;
        let j4 = discord::resolve_user_arg(options.get(4).unwrap())?;
        let score_j3j4 = discord::resolve_int_arg(options.get(5).unwrap())?;

        let middleware = BabyfootMiddleware::new(app);

        debug!("Executing BabyfootMiddleware::register_match_2v2 middleware...");
        let content = middleware
            .register_match_2v2(j1, j2, j3, j4, i32::try_from(score_j1j2)?, i32::try_from(score_j3j4)?)
            .await?;
    ...
    fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
        command
            .name("babyfoot_2v2")
            .description("Enregistrer un match de babyfoot en 2v2")
            .create_option(|option| {
                option
                    .name("j1")
                    .description("Premier joueur du premier groupe")
            ...
            .create_option(|option| {
                option
                    .name("12")
                    .description("Second joueur du premier groupe")
                    .kind(CommandOptionType::User)
                    .required(true)
            })
            .create_option(|option| {
                option
                    .name("score-j1-j2")
                    .description("Score du premier groupe")
